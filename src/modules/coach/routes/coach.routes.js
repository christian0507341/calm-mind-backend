...
router.get('/context', async (req, res) => {
  try {
    ...
    const prevWeekLogs = await StressDaily.find({
      user_id,
      date_local: { $gte: now.minus({ days: 14 }).startOf('day').toJSDate(), $lt: weekAgo }
    });

    const today_avg = todayDaily?.avg_level || null;
    const week_avg = weekLogs.length ? weekLogs.reduce((sum, d) => sum + d.avg_level, 0) / weekLogs.length : null;
    const prev_week_avg = prevWeekLogs.length ? prevWeekLogs.reduce((sum, d) => sum + d.avg_level, 0) / prevWeekLogs.length : null;
    const trend = week_avg && prev_week_avg ? (week_avg - prev_week_avg > 0.5 ? 'up' : week_avg - prev_week_avg < -0.5 ? 'down' : 'flat') : 'flat';
    ...
    const next_deadlines = tasks
      ...
      .slice(0, 3);
    const tasks_completed_today = tasks.filter(t => t.status === 'Completed' && DateTime.fromJSDate(new Date(t.created_at)).hasSame(now, 'day')).length;
...
router.post('/chat', async (req, res) => {
  try {
    ...
    const contextRes = await fetch(`http://localhost:${process.env.PORT || 4000}/api/coach/context?user_id=${user_id}`, {
      headers: { Authorization: req.headers.authorization }
    });
    const context = await contextRes.json();

    const { response, stress_band, tone } = await getLLMResponse(context, message);

    let finalResponse = response;
    if (stress_band === 5 && /distress|hopeless|suicide/i.test(message)) {
      finalResponse += '\n\nIf you’re feeling overwhelmed, reach out: [Crisis Text Line](https://www.crisistextline.org) or a trusted advisor.';
    }
    ...
    res.json({
      ...
      stress_band,
      tone,
      buttons: stress_band <= 3 ? ['Apply Plan', 'Split Task'] : []
    });
  } catch (err) {
